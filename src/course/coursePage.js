...
export default function CoursePage() {
    ...
    const handleChangeSemester = async (event) => {
        await fetch(process.env.REACT_APP_API_BASEURL + URLPath + query + `/${event.target.value}`).then(resp => resp.json()).then((data) => setSemester(data)).catch(() => {
    ...
    const handleChangeSection = async (event) => {
        await fetch(process.env.REACT_APP_API_BASEURL + URLPath + query + `/${semesterSelected}/${event.target.value}`).then(resp => resp.json()).then((data) => setSection(data)).catch(() => {
            window.location.href = "/"
        });
    ...
    useEffect(async () => {
        await fetch(process.env.REACT_APP_API_BASEURL + URLPath + query).then(resp => resp.json()).then((data) => setCourse(data)).catch(() => {
    ...
    return (
        <div ref={ref}>
            <CircularProgress style={{display: course ? "none" : "block", margin: "20px auto"}}/>
                ...
                <Box p={1}>
                    ...
                    >
                        <Typography variant="h3" gutterBottom
                                    className={classes.title}>{course ? course.subject + " " + course.catalogNbr + ": " + course.title : ""}</Typography>
                        <FormControl variant="outlined" className={classes.formControl}>
                            <InputLabel id="simple-select-outlined-label" ref={ref}>Semester</InputLabel>
                            <Select ref={ref}
            ...
            <Box display="flex" className={classes.box} justifyContent="flex-start" m={1} p={1}>
                <Grid
                    ...
                    alignItems="flex-start"
                >
                    <Typography variant="h4" gutterBottom className={classes.title}>Classes: </Typography>
                    <FormControl variant="outlined" className={classes.formControl}>
                        <InputLabel id="simple-select-outlined-label" ref={ref}>Section</InputLabel>
                        <Select ref={ref}
                                labelId="simple-select-outlined-label"
                                id="simple-select-outlined"
                    ...
                    <TableBody>
                        {section.meetingPatterns.map((meetingPattern) => (
                            <TableRow key={meetingPattern.meetingNumber}>
                                ...
                                <TableCell
                                    align="right">{meetingPattern.daysOfWeek + "  " + meetingPattern.startTime.substring(0, 5) + " -- " + meetingPattern.endTime.substring(0, 5)}</TableCell>
